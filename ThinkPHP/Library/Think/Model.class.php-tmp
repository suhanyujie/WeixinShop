...
// 动态指定表名
    ...
    public function table($table) {
        ...
        if (is_array($table)) {
            ...
            $table = preg_replace_callback("/__([A-Z0-9_-]+)__/isU", function ($match) use ($prefix) {return $prefix . strtolower($match[1]);}, $table);
            $this->options['table'] = $table;
        }
        return $this;
    }
    ...
    public function using($using) {
        ...
        if (is_array($using)) {
            ...
            //将__TABLE_NAME__替换成带前缀的表名
            $using = preg_replace_callback("/__([A-Z0-9_-]+)__/isU", function ($match) use ($prefix) {return $prefix . strtolower($match[1]);}, $using);
            $this->options['using'] = $using;
        }
        return $this;
    }
    ...
    public function join($join, $type = 'INNER') {
        ...
        if (is_array($join)) {
            foreach ($join as $key => &$_join) {
                $_join = preg_replace_callback("/__([A-Z0-9_-]+)__/isU", function ($match) use ($prefix) {return $prefix . strtolower($match[1]);}, $_join);
                $_join = false !== stripos($_join, 'JOIN') ? $_join : $type . ' JOIN ' . $_join;
            }
            $this->options['join'] = $join;
        } elseif (!empty($join)) {
            //将__TABLE_NAME__字符串替换成带前缀的表名
            $join = preg_replace_callback("/__([A-Z0-9_-]+)__/isU", function ($match) use ($prefix) {return $prefix . strtolower($match[1]);}, $join);
    ...
    public function union($union, $all = false) {
        ...
        if (is_string($union)) {
            $prefix = $this->tablePrefix;
            //将__TABLE_NAME__字符串替换成带前缀的表名
            $options = preg_replace_callback("/__([A-Z0-9_-]+)__/isU", function ($match) use ($prefix) {return $prefix . strtolower($match[1]);}, $union);
...
// 字段排除
        ...
        $prefix = $this->tablePrefix;
        $field = preg_replace_callback("/__([A-Z0-9_-]+)__/isU", function ($match) use ($prefix) {return $prefix . strtolower($match[1]);}, $field);
    ...
    public function where($where, $parse = null) {
        if (is_string($where)) {
            $prefix = $this->tablePrefix;
            $where = preg_replace_callback("/__([A-Z0-9_-]+)__/isU", function ($match) use ($prefix) {return $prefix . strtolower($match[1]);}, $where);
        }

...
class CollectAlimamaAction extends \Common\Action\backendAction {
    ...
    public function batch_publish() {
        if (IS_POST) {
            ...
            //采集页数
            $page_num = input('post.page_num', 'intval', 10);
            //获取马甲
            ...
            //搜索条件
            $form_data = input('post.form_data', 'urldecode');
    ...
    public function search() {
        ...
        if (input('get.search')) {
            $map['keyword'] = input('get.keyword', 'trim');//关键词
            $map['cid']     = input('get.cid', 'intval');//分类ID
            $p              = input('get.p', 'intval', 1);
            //淘宝只提供400数据
            if ($p > 10) {
            ...
            $map['start_price']          = input('get.start_price', 'trim');//价格下限
            $map['end_price']            = input('get.end_price', 'trim');//价格上限
            $map['start_commissionRate'] = input('get.start_commissionRate', 'trim');//佣金比率下限
            $map['end_commissionRate']   = input('get.end_commissionRate', 'trim');//佣金比率上限
            $map['start_commissionNum']  = input('get.start_commissionNum', 'intval');//30天推广量下限
            $map['end_commissionNum']    = input('get.end_commissionNum', 'intval');//30天推广量上限
            $map['start_totalnum']       = input('get.start_totalnum', 'intval');//总销量下限
            $map['end_totalnum']         = input('get.end_totalnum', 'intval');//总销量上限
            $map['start_credit']         = input('get.start_credit', 'trim');//卖家信用下限
            $map['end_credit']           = input('get.end_credit', 'trim');//卖家信用上限
            $map['mall_item']            = input('get.mall_item', 'intval');//是否天猫商品
            $map['guarantee']            = input('get.guarantee', 'intval');//是否消保卖家
            $map['sevendays_return']     = input('get.sevendays_return', 'intval');//是否支持7天退换
            $map['real_describe']        = input('get.real_describe', 'intval');//是否先行赔付
            $map['cash_coupon']          = input('get.cash_coupon', 'intval');//是否支持抵价券
            $map['sort']                 = input('get.sort', 'trim');//排序方法

...
class KeywordAction extends \Common\Action\backendAction {
    ...
    public function addkeyword()//关键词
    {
        ...
        } else { $keyinfo = M('keyword')->where('iskey=1')->order('kid desc')->select();
    ...
    public function ajax_form() {
        ...
        $data['kid']       = input('post.id', 'intval');
        $kename            = input('post.kename', 'trim');
        $data['kecontent'] = input('post.kecontent', 'trim');
        $kyword            = input('post.kyword', 'trim');

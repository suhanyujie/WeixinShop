...
class MessageAction extends \Common\Action\backendAction {
    ...
    protected function _search() {
        ...
        ($keyword = input('request.keyword', 'trim')) && $map['info']             = array('like', '%' . $keyword . '%');
        ($from_name = input('request.from_name', 'trim')) && $map['from_name']    = array('like', '%' . $from_name . '%');
        ($to_name = input('request.to_name', 'trim')) && $map['to_name']          = array('like', '%' . $to_name . '%');
        $type                                                                     = input('request.type', 'intval');
        if ($type) {
            if ($type == 1) {
                $map['from_id'] = 0;
            } else if ($type == 2) {
    ...
    public function add() {
        if (IS_POST) {
            //内容
            $type = input('post.type', 'trim', 'custom');
            //用户
            $to_name = input('post.to_name', 'trim');
            //发送者
            ...
            if ($type == 'custom') {
                //自定义
                $info = input('post.info', 'trim');
                !$info && $this->ajaxReturn(0, L('message_empty'));
            } else {
                //获取模板名称
                $tpl_alias = input('post.tpl_alias', 'trim');
            }
            //逐条发送

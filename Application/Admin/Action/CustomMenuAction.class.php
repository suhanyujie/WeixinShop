...
class CustomMenuAction extends \Common\Action\backendAction {
    ...
    public function index() {
        $sort = input("get.sort", 'trim', 'ordid');
        $order = input("get.order", 'trim', 'ASC');
    ...
    protected function _before_update($data = '') {
        ...
        if ($data['pid'] != $item_cate['pid']) {
            ...
            $wp_spid_arr = $this->_mod->getChildIds($data['id'], true);
    ...
    public function ajax_tag_edit() {
        $tag_id = input('get.id', 'intval');
        $cate_id = input('get.cate_id', 'intval');
        if (!$cate_id && !$tag_id) {
            $this->ajaxReturn(0, L('illegal_parameters'));
        }
        $weight = input('get.val', 'intval', 0);
    ...
    public function tag_add() {
        if (IS_POST) {
            ...
            $tag_ids = input('post.tag_ids', 'trim');
            $custom_tags = input('post.custom_tags', 'trim');
            $tag_ids_arr = array();
            if ($tag_ids) {
    ...
    public function ajax_getchilds() {
        ...
        $type = input('get.type', 'intval', null);
        $map = array('pid' => $id);
        if (!is_null($type)) {
            $map['type'] = $type;
        }

...
class CmtTaobaoAction extends \Common\Action\backendAction {
    ...
    public function setting() {
        $cate_id = input('post.cate_id', 'intval');//分类
        $priority = input('post.priority', 'trim');//优先级
        $pagesize = input('post.pagesize', 'intval');//批量
        $cmt_num = input('post.cmt_num', 'intval');//评论数
        $auid = input('post.auid', 'intval');//马甲
        $rate_type = input('post.rate_type', 'intval');//淘宝评论类型
        $sort_type = input('post.sort_type', 'trim');//淘宝评论排序
        ...
        $id_arr = D('item_cate')->getChildIds($cate_id, true);
    ...
    private function _collect_one($iid, $item_id, $cmt_num, $users) {
        ...
        if ($seller['type'] == 'tmall') {
            ...
            $source = \Common\ORG\Http::fsockopenDownload($rate_tmall_api);
            ...
            $source = \Common\ORG\Http::fsockopenDownload($rate_taobao_api);
            $source = rtrim(ltrim(trim($source), '('), ')');
            $source = iconv('GBK', 'UTF-8//IGNORE', $source);
            $rate_resp = json_decode($source, true);

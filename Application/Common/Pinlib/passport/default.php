 ...
 */
namespace Common\Pinlib\passport;
class defaultPassport {
    ...
    public function auth($username, $password) {
        $user = M('user')->field('id,password')->where(array('username' => $username))->find();
        if ($user) {
            ...
            $user = M('user')->field('id,password')->where(array('mobile' => $username))->find();
            if ($user['password'] != md5($password)) {
                $this->_error = L('auth_failed');
                return false;
            }

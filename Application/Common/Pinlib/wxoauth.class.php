...
class wxoauth {
    ...
    public function __construct($type) {
        ...
        //导入接口文件
        include_once COMMON_PATH . 'Pinlib/' . $type . '/' . $type . '.php';
        $om_class  = '\Common\Pinlib\\' . $type . '\\' . $type . '_oauth';
    ...
    public function callbackbinduser($request_args) {
        ...
        $user      = $this->_om->getUserInfo($request_args);
        $checkuser = M('user')->field('id,wechatid,mobile,mobile_valid,nickname,gender,mobile,mobile_valid,province,city')->where("wechatid='" . $user['keyid'] . "'")->find();
        if ($checkuser) {
            $date = time();
            ...
            $user = M('user')->field('id,username,wechatid,nickname,mobile,mobile_valid,gender,province,city')->where("wechatid='" . trim($user['keyid']) . "'")->find();
            if ($user) {
                $data['last_time'] = time();
        ...
        if ($checkuser && $checkuser['mobile_valid'] == 1) {
            $_SESSION['user_info'] = $checkuser;
            return U('User/index');
        }
        return U('User/verifyMobile');
    }
    /**
     * 更新绑定信息
     */

...
class UserVisitor {
    ...
    public function __construct() {
        ...
        if (isset($_GET['key'])) {
            $user = M('user')->field('id,username,wechatid,mobile,mobile_valid,gender,nickname,province,city')->where("wechatid='" . trim($_GET['key']) . "'")->find();
            if ($user) {
                $data['last_time'] = time();
        ...
        if (session('user_info')) {
            ...
            $user_info = M('user')->field('id,username,wechatid,mobile,mobile_valid,gender,nickname,province,city')->where(array('id' => $user_info['id'], 'password' => $user_info['password']))->find();
            if ($user_info) {
                //记住登陆状态
    ...
    public function login($uid, $remember = null) {
        ...
        $user_info             = $user_mod->field('id,username,password,nickname,wechatid,gender,province,city,nickname,mobile,mobile_valid')->find($uid);

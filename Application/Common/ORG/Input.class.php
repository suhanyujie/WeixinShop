...
/** 输入数据管理类
 * 使用方法
 *  $Input = \Common\ORG\Input::getInstance();
 ...
 *
 * 获取数据：
 *    如果从$_POST或者$_GET中获取，使用\Common\ORG\Input::getVar($_POST['field']);，从数据库或者文件就不需要了。
 *    或者直接使用 \Common\ORG\Input::magicQuotes来消除所有的magic_quotes_gpc转义。
 *
 * 存储过程：
 *    经过\Common\ORG\Input::getVar($_POST['field'])获得的数据，就是干净的数据，可以直接保存。
 *    如果要过滤危险的html，可以使用 $html = \Common\ORG\Input::safeHtml($data);
 *
 * 页面显示：
 *    纯文本显示在网页中，如文章标题<title>$data</title>： $data = \Common\ORG\Input::forShow($field);
 *    HTML 在网页中显示，如文章内容：无需处理。
 *    在网页中以源代码方式显示html：$vo = \Common\ORG\Input::forShow($html);
 *    纯文本或者HTML在textarea中进行编辑: $vo = \Common\ORG\Input::forTarea($value);
 *    html在标签中使用，如<input value="数据" /> ，使用 $vo = \Common\ORG\Input::forTag($value); 或者 $vo = \Common\ORG\Input::hsc($value);
 *
 * 特殊使用情况：
 *    字符串要在数据库进行搜索： $data = \Common\ORG\Input::forSearch($field);
 */
...
class Input {
    ...
    static public function makeLink($string) {
        ...
        $replacements = array(
            "'\\1<a href=\"\\2://\\3\" title=\"\\2://\\3\" rel=\"external\">\\2://'.\Common\ORG\Input::truncate( '\\3' ).'</a>'",
            "'\\1<a href=\"http://www.\\2.\\3\" title=\"www.\\2.\\3\" rel=\"external\">'.\Common\ORG\Input::truncate( 'www.\\2.\\3' ).'</a>'",
            "'\\1<a href=\"ftp://ftp.\\2.\\3\" title=\"ftp.\\2.\\3\" rel=\"external\">'.\Common\ORG\Input::truncate( 'ftp.\\2.\\3' ).'</a>'",
            "'\\1<a href=\"mailto:\\2@\\3\" title=\"\\2@\\3\">'.\Common\ORG\Input::truncate( '\\2@\\3' ).'</a>'");

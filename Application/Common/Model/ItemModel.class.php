...
class ItemModel extends Model {
    ...
    /**
     * 发布一个商品
     * @param mixed $item 商品信息
     * @param integer $album_id 专辑ID
     * @param integer $ac_id 专辑分类ID
     * @param boolean $admin 是否是后台添加商品，后台添加时不需要产品专辑
     */
    public function publish($item, $album_id = 0, $ac_id = 0, $admin = false) {
        //已经存在？
        ...
        unset($item['add_time']);
        $data    = $this->create($item, self::MODEL_INSERT);
        $item_id = $this->add($data);
        if ($item_id) {
            //商品相册处理

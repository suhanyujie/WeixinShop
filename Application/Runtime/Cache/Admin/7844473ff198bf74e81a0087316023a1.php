...
<head>
	...
	var SELF = '/index.php?g=admin&m=item&a=crawler&menuid=327';
	var ROOT_PATH = '';
	var APP	 =	 '/index.php';
	//语言项目
	var lang = new Object();
...
<form id="info_form" action="<?php echo u('item/crawler');?>" method="post" enctype="multipart/form-data">
    <div class="pad_lr_10">
        <div class="col_tab">
            ...
            -->
                            ...
                            <tr>
                                <th>商品图片 :</th>
                                <td>
                                    <input type="hidden" name="remote_image" id="J_remote_image" class="input-text" size="60">
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php if(is_array($cate)): $i = 0; $__LIST__ = $cate;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cate): $mod = ($i % 2 );++$i;?><span style="display:inline-block" id='J_user_price'>
                                            ...
                                            <input class='user_price' onkeyup="this.value=this.value.replace(/[^0-9.\-]/g,'')" onafterpaste="this.value=this.value.replace(/[^0-9\-.]/g,'')" type="text" name="user_price[price][]" class="input-text" size="10" value="-1">&nbsp</span><?php endforeach; endif; else: echo "" ;endif; ?>
                                    <br/>
                ...
                <td><input id='J_goods_stock' onkeyup="this.value=this.value.replace(/[^0-9]/g,'')" onafterpaste="this.value=this.value.replace(/[^0-9]/g,'')" type="text" name="goods_stock" class="input-text" size="10"> </td>
                            ...
                            <tr>
                                <th>商品详情 :</th>
                                <td>
                                    <script type="text/plain" name="info" id="info"></script>
                                    <span id="showInfoEditor">显示编辑器</span>
                                </td>
                            </tr>
                    </table>
                </div>
...
$(function() {
    ...
    $('#J_barcode').on('blur', function(e) {
        if ($.trim($(this).val()) != '') {
            ...
            $.getJSON('<?php echo U("item/getProductInfo");?>', {
                barcode: barcode
            }, function(result) {
                if (result.status == 1 && result.content != '') {
                    $('#J_remote_image').val(result.image);
                    $('#J_title').val(result.title);
                    $('#info').html(result.content);
                    $('#J_price').val(result.price);
    ...
    $('#showInfoEditor').click(function(event) {
        var ue = UE.getEditor('info', {
            serverUrl: "<?php echo U('Admin/Item/ueditor');?>",
            textarea: 'info',
            initialFrameWidth: 600,
